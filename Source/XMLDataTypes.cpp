...
template <typename T> void AttributeBase::setData(T data)
{
    // template acrobatics, baby!
    dynamic_cast<Attribute<T>*>(this)->setData(data);
}

template <typename T> T AttributeBase::getData()
{
    // more acrobatics
    return dynamic_cast<Attribute<T>&>(*this).getData();
}

template <typename T> void AttributeBase::swapDataPointer(T* newPtr)
{
    dynamic_cast<Attribute<T>*>(this)->swapDataPointer(newPtr);
}

template <typename T> T* AttributeBase::getDataPointer()
{
...
template <> std::string Attribute<std::string>::getDataString()
{
    return *m_data;
}
template <> std::string Attribute<unsigned int>::getDataString()
{
    return std::to_string(*m_data);
}
template <> std::string Attribute<int>::getDataString()
{
    return std::to_string(*m_data);
}
template <> std::string Attribute<float>::getDataString()
{
    return std::to_string(*m_data);
}
template <> std::string Attribute<bool>::getDataString()
{
    return std::to_string(*m_data);
}
template <> std::string Attribute<glm::vec2>::getDataString()
{
    // "{x}{y}"
    return std::string("{" + std::to_string(m_data->x) + "," + std::to_string(m_data->y) + "}");
}
template <> std::string Attribute<std::vector<std::string>>::getDataString()
{
    std::string ret = "";
    for(auto str : *m_data)
	{
...
template <> std::string Attribute<std::vector<unsigned int>>::getDataString()
{
    std::string ret = "";
    for(auto e : *m_data)
	{
...
template <> std::string Attribute<std::vector<int>>::getDataString()
{
    std::string ret = "";
    for(auto e : *m_data)
	{
...
template <> std::string Attribute<std::vector<float>>::getDataString()
{
    std::string ret = "";
    for(auto e : *m_data)
	{
...
template <> std::string Attribute<std::vector<bool>>::getDataString()
{
    std::string ret = "";
    for(auto e : *m_data)
	{
...
template <> std::string Attribute<std::vector<glm::vec2>>::getDataString()
{
    std::string ret = "";
    for(auto e : *m_data)
	{
...
template <> std::string Attribute<LuaScript>::getDataString()
{
    return m_data->m_path;
}
template <> std::string Attribute<Texture>::getDataString()
{
    return m_data->filepath;
}

// SetDefaultData
template <> void Attribute<std::string>::setDefaultData()
{
    setData("");
}
template <> void Attribute<unsigned int>::setDefaultData()
{
    setData((unsigned int)-1);
}
template <> void Attribute<int>::setDefaultData()
{
    setData(-1);
}
template <> void Attribute<float>::setDefaultData()
{
    setData(0.0f);
}
template <> void Attribute<bool>::setDefaultData()
{
    setData(0);
}
template <> void Attribute<glm::vec2>::setDefaultData()
{
    setData(glm::vec2(0.0f, 0.0f));
}
template <> void Attribute<std::vector<std::string>>::setDefaultData()
{
    setData(std::vector<std::string>{});
}
template <> void Attribute<std::vector<unsigned int>>::setDefaultData()
{
    setData(std::vector<unsigned int>{});
}
template <> void Attribute<std::vector<int>>::setDefaultData()
{
    setData(std::vector<int>{});
}
template <> void Attribute<std::vector<float>>::setDefaultData()
{
    setData(std::vector<float>{});
}
template <> void Attribute<std::vector<bool>>::setDefaultData()
{
    setData(std::vector<bool>{});
}
template <> void Attribute<std::vector<glm::vec2>>::setDefaultData()
{
    setData(std::vector<glm::vec2>{});
}
template <> void Attribute<LuaScript>::setDefaultData()
{
    setData(LuaScript());
}
template <> void Attribute<Texture>::setDefaultData()
{
    Texture tex;
    tex.filepath = "";
    setData(tex);
...
void XMLData::read(rapidxml::xml_node<>* node)
{
    ...
    for(auto& attr : m_attributes)
	{
	    ...
	    Logger::getInstance()->log("\t\tReading attribute " + attrName);
		...
		case AttributeType::TEXTURE:
			...
			if(path == "")
			    {
				// The data isn't available - default to an empty texture
			} else
			    {
				// Just load it!
				tex = ResourceManager::loadTexture(path);
			    }
			// Set the data
		...
		default:
		    {
			throwError(BAREError::XML_FAILURE, "Failed to read attribute with type: " +
			                                       std::to_string((unsigned int)attrValue->type) +
			                                       " of name " + attrName);
			return;
		    }
		};
...
void XMLData::write(rapidxml::xml_document<>* doc)
{
    ...
    for(auto& attr : m_attributes)
	{
	    ...
	    // We don't even really need to interpret (exterpret?) the value, as converting from data to a string is
	    // defined by the actual attribute, not by the type.
	    ...
	    // Allocate the node
	    rapidxml::xml_node<>* allocatedNode =
	        doc->allocate_node(rapidxml::node_element, allocatedName, allocatedValue);
