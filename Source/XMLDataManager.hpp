...
class XMLDataManager
{
  ...
  public:
    ...
    /**
     * @brief Writes all of the currently cached data to the data's respective files in a folder at filepath. Does not
     * clear caches.
     ...
     */
    template <typename T> static void addDataType();
    ...
    /**
     * @brief Adds or overwrites data in the cache/sub-cache
     * @param data A pointer to the data to be added. data->nodeName must be set for appropriate sub-caching
     */
     ...
     */
    template <typename T> static T getData(std::string dataType, unsigned int key);
  ...
  private:
     ...
     */
    template <typename T> static bool readValue(rapidxml::xml_node<>* parent, std::string valueName, T& variable);
