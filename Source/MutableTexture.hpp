...
namespace BARE2D {
	...
	struct MutableTexture : public Texture
	{
		 ...
		 */
		void setData(unsigned char* data, GLenum format = GL_RGBA);
		
		/**
		 * @brief Sets the data of the mutable texture, as well as updates the OpenGL context's texture data for it (uses glTextureSubImage2D).
		 * @param data The data (formatted R0,G0,B0,A0,R1,G1,B1,A0,etc.).
		 ...
		 * @param format The format of the data. Must follow OpenGL Formats (GL_RED, GL_RG, etc.). Defaults to GL_RGBA
		 */
		void setData(unsigned char* data, unsigned int xOffset, unsigned int yOffset, unsigned int subWidth, unsigned int subHeight, GLenum format = GL_RGBA);
	};
