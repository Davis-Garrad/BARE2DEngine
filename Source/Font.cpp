...
namespace BARE2D {
	...
	void Font::init(const char* fontFile, int size)
	{
		...
		for(i = 0; i < m_regLength; i++) {
			...
			m_characterGlyphs[i].uvRect = glm::vec4((float)glyphRectangles[i].x / (float)bestWidth,
													(float)glyphRectangles[i].y / (float)bestHeight,
													(float)glyphRectangles[i].z / (float)bestWidth,
													(float)glyphRectangles[i].w / (float)bestHeight);
		}
		...
		m_characterGlyphs[m_regLength].uvRect = glm::vec4(0.0f, 0.0f, (float)zero_index / (float)bestWidth, (float)zero_index / (float)bestHeight);
	...
	glm::vec2 Font::measure(const char* s)
	{
		...
		// It is fixed height, so we just need to calculate the x value of the size, plus any newlines that contribute to height
