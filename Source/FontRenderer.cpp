...
FontRenderer::FontRenderer(std::string& fragShader,
...
void FontRenderer::draw(Font& font,
                        glm::vec2 scaling,
                        glm::vec4 destRect,
                        const char* text,
                        float depth,
                        ...
                        glm::vec4 uvRect /**= glm::vec4(0.0f, 0.0f, 1.0f, 1.0f)*/)
{
    ...
    for(int characterIndex = 0; text[characterIndex] != 0; characterIndex++)
	{
	    ...
	    } else
		{
		    ...
		    glm::vec4 updatedDestRect(newPosition, glyph->size * scaling);
		    glm::vec4 uvRectFinal(glyph->uvRect.x + uvRect.x * glyph->uvRect.z,
		                          glyph->uvRect.y + uvRect.y * glyph->uvRect.w, glyph->uvRect.z * uvRect.z,
		                          glyph->uvRect.w * uvRect.w);

		    // Actually draw the texture!
		    BasicRenderer::draw(updatedDestRect, uvRectFinal, font.getTextureID(), depth);
