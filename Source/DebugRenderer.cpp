...
const char* fragShaderSource = "#version 130\n"
...
const char* vertShaderSource = "#version 130\n"
...
namespace BARE2D {
	...
	void DebugRenderer::init() {
		...
		link({"vertexPosition", "vertexColour"});
	...
	void DebugRenderer::createRenderBatches() {
		...
		circleVertices.resize(m_circles.size() *
							  CIRCLE_LINES); // we only actually use one vertex per line, as OpenGL fills in the rest.

		std::vector<Vertex> rectangleVertices;
		rectangleVertices.resize(m_rectangles.size() *
								 4); // OpenGL uses 4 vertices for a polygon, so that's what we'll do.
		...
		for(unsigned int i = 0; i < m_circles.size(); i++) {
			...
			for(unsigned int vertex = 1; vertex < CIRCLE_LINES; vertex++) {
				// Set the position
				circleVertices[vertex + CIRCLE_LINES * i].setPosition(m_circles[i].lines[vertex].point0.x,
																	  m_circles[i].lines[vertex].point0.y,
																	  0.0f);
		...
		for(unsigned int i = 0; i < m_rectangles.size(); i++) {
			// Set the positions
			rectangleVertices[4 * i].setPosition(m_rectangles[i].lines[0].point0.x,
												 m_rectangles[i].lines[0].point0.y,
												 0.0f);
			rectangleVertices[4 * i + 1].setPosition(m_rectangles[i].lines[0].point1.x,
													 m_rectangles[i].lines[0].point1.y,
													 0.0f);
			rectangleVertices[4 * i + 2].setPosition(m_rectangles[i].lines[1].point0.x,
													 m_rectangles[i].lines[1].point0.y,
													 0.0f);
			rectangleVertices[4 * i + 3].setPosition(m_rectangles[i].lines[1].point1.x,
													 m_rectangles[i].lines[1].point1.y,
													 0.0f);
