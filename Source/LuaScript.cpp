...
namespace BARE2D {
	...
	void LuaScriptContextWrapper::start() {
		...
		if(errorCode != LUA_OK && errorCode != LUA_YIELD) {
			...
			throwError(BAREError::LUA_FAILURE,
					   "Failed to run Lua script: Lua reports: Error code " + std::to_string(errorCode) + " - \'" +
	...
	void LuaScriptContextWrapper::loadLua(std::string& luaStr) {
		...
		if(errorCode != LUA_OK) {
			// There is some error. Throw, and use lua_tostring to get the error string.
			throwError(BAREError::LUA_FAILURE,
					   "Failed to compile Lua string: Lua reports: Error code " + std::to_string(errorCode) + " - \'" +
						   lua_tostring(m_state, -1) + "\'\n\n" + luaStr + "\n\n");
