...
void OrbitScreen::draw()
{
    ...
    m_regrenderer->draw(glm::vec4(-1.0f, -1.0f, 2.0f, 2.0f), glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), m_starsTexture.id, 0.0f,
                        BARE2D::Colour(255, 255, 255, 128));
	...
	{
	    glm::vec4 destRect =
	        glm::vec4(m_bodies[i]->pos.x, m_bodies[i]->pos.y, m_bodies[i]->radius * 2, m_bodies[i]->radius * 2);
...
int OrbitScreen::getNextScreenIndex() const
{
    return 0;
}
...
void OrbitScreen::update(double dt)
{
    ...
    if(m_input->isKeyPressed(SDL_BUTTON_RIGHT) ||
       (m_input->isKeyDown(SDL_BUTTON_RIGHT) && m_input->isKeyDown(SDLK_LCTRL)))
	{
	    m_bodies.push_back(
	        new Body(m_renderer->getCamera()->getViewspaceCoord(
	                     m_input->getMousePosition() -
	                     glm::vec2(m_window->getWidth() / 2.0f, -(float)(m_window->getHeight() / 2.0f))),
	                 glm::vec2(0.0f, 0.0f), 7.30e17, 1.7e3));
    }
